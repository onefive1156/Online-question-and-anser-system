...
export default {
	init() {
			...
			if(uniIm.socketOpenIndex > 1){
				...
				setTimeout(async()=> {
					...
					let res = await db.collection('uni-im-msg')
									...
									.get()
					console.log('获取切到后台后socket离线丢失的数据 res',maxConversation.update_time,res.result.data);
					let clodMsgData = {}
		...
		function setIsWidescreen() {
			...
			if (oldState == false && uniIm.isWidescreen == true) {
				if (!window.location.href.includes('/uni_modules/uni-im/pages/index/index')) {
					console.log('uni-im检测到窗口由小屏切为大屏，已重定向。')
					uni.showModal({
		...
		uniIm.audioContext = uni.createInnerAudioContext()
		...
		uni.onPushMessage(async res => {
			...
			if (payload.type == "uni-im") {
				...
				if (res.type == 'receive') {
					...
					//限制群聊才有回复提示
					if (msg.about_msg_id && msg.group_id) {
					...
					if (topViewRoute == 'uni_modules/uni-im/pages/chat/chat') {
						uni.redirectTo({
							url: '/uni_modules/uni-im/pages/chat/chat?conversation_id=' + msg
								.conversation_id,
							complete(e) {
								console.log(e);
							}
