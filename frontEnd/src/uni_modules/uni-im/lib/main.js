	...
	conversation: {
		...
		async revokeMsg({
			...
			if (!user_id) { // 为空表示当前用户就是操作者
				uniImCo.revokeMsg(msg_id).then(e => {
					// console.log('revokeMsg ---> ',e)
				}).catch((e) => {
					console.error('撤回消息命令未同步到云端', e);
				})
			}
			...
			if (param) {
				if (typeof param == 'object') {
					...
					if (!conversationId) {
						...
						conversationId = utils.getConversationId(friend_uid || group_id, friend_uid ? 'single' :
							'group')
					}
