<template>
	<view class="root">
		...
		<view v-if="call_list.length" style="position: fixed;right: 5px;top: 200px;">
			<button @click="showCallMe">回复我的</button>
...
<script>
	...
	export default {
		...
		computed: {
			...
			listHeight(){
				// #ifdef APP-NVUE
					// console.log("this.systemInfo.windowHeight - parseInt(paddingBottom) +'px'",this.systemInfo.windowHeight - parseInt(this.paddingBottom) - 25 +'px')
				return this.systemInfo.windowHeight - parseInt(this.paddingBottom) - 83 +'px';
				// #endif
				return 'auto'
			}
		},
		...
		methods: {
			...
			doTasksList() {
				...
				this.tasksList.forEach(async fun => await fun())
				this.tasksList = []
			},
