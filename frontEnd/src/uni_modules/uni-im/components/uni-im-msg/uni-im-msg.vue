			...
			<template v-else>
				...
				<view :class="{'reverse-align':self}">
					...
					<view class="msg-content" @longpress="showControl" ref="msg-content">
						...
						<cloud-image width="200rpx" height="200rpx" @click="previewImage(msg.body.url||msg.body)"
							v-if="msg.type == 'image'" :src="msg.body.url" mode="aspectFill" />
						<view @click="playSound(msg.body.url)" v-if="msg.type == 'sound'"
	...
	export default {
		...
		async mounted() {
			...
			if (this.msg.type == 'video') {
				let res = await uniCloud.getTempFileURL({
					fileList: [this.msg.body.url]
				})
				this.videoUrl = res.fileList[0].tempFileURL
			} else if (this.msg.type == 'sound') {
				audioContext.onPlay(async () => {
					// console.log('soundPlayStart------------------');
					let src = uniIm.audioContext.src
					let audioUrlRes = await uniCloud.getTempFileURL({
						fileList: [this.msg.body.url]
					})
					if (src == audioUrlRes.fileList[0].tempFileURL) {
		...
		methods: {
			...
			async playVideo() {
				...
				// 腾讯云版uniCloud的文件需要获取临时路径
				if (this.videoUrl.substring(0, 8) == "cloud://") {
					let res = await uniCloud.getTempFileURL({
			...
			async downLoadFile() {
				...
				// 腾讯云版uniCloud的文件需要获取临时路径
				if (this.msg.body.url.substring(0, 8) == "cloud://") {
					let res = await uniCloud.getTempFileURL({
						fileList: [this.msg.body.url]
					})
					this.msg.body.url = res.fileList[0].tempFileURL
				}
				uni.downloadFile({
