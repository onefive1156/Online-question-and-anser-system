...
module.exports = {
	/* 本地调试 使用url化的uni-push 方便测试 */
	async pushByDebug() {
		console.error('注意本云方法，仅用于测试期间：方便本地调试执行push使用。存在安全隐患，正式上线后请删除')

		// console.error('本地调试使用push的方法被禁用，如要开启，请注释或删除本行代码')

		// console.log('this.getHttpInfo().body',this.getHttpInfo().body)
		let {
			appId,
			pushParam
	...
	async _before() {
		...
		this.sendPushMsg = async (param, appId) => {
			...
			if (this.clientInfo.clientIP == "127.0.0.1" || this.clientInfo.clientIP.includes(
				"192.168")) {
				console.error('检测到当前为本地调试，消息推送将通过url化后的云对象执行（执行速度相比正式项目较慢）')
				let url =
					"https://fc-mp-3695f427-280f-498a-90ec-0997e7427f30.next.bspapp.com/uni-im-co/pushByDebug" //请填写url化后的地址
				if (url.length === 0) {
					console.error('请检查：url链接是否与你在web控制台（https://unicloud.dcloud.net.cn/）配置的一致')
				}
